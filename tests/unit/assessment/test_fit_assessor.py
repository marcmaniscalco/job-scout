...
def test_assess_builds_fit_assessments_with_model_id():
    ...
    bedrock_client.assess_fit.return_value = {
        "job_fit": {"rating": "Strong", "reasoning": "Strong match"},
        "compensation_fit": {"rating": "Fair", "reasoning": "Below market"},
    }
    ...
    assert result["job_fit"] == FitAssessment(
        rating="Strong", reasoning="Strong match", model_id="haiku-4-5"
    )
    assert result["compensation_fit"] == FitAssessment(
        rating="Fair", reasoning="Below market", model_id="haiku-4-5"
    )
